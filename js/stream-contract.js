...
/* Stream lifecycle. Heartbeat proves CONNECTION alive; only market events
   ...
   Handshake states may enter RECONNECTING: a socket that dies mid-handshake
   (connect/auth/subscribe) while subscriptions exist is a failed attempt in
   the bounded retry cycle, not a terminal failure. */
...
const TRANSITIONS = Object.freeze({
  ...
  [STREAM_STATES.CONNECTING]: [STREAM_STATES.AUTHENTICATING, STREAM_STATES.RECONNECTING, STREAM_STATES.FAILED, STREAM_STATES.CLOSED],
  [STREAM_STATES.AUTHENTICATING]: [STREAM_STATES.SUBSCRIBING, STREAM_STATES.RECONNECTING, STREAM_STATES.FAILED, STREAM_STATES.CLOSED],
  [STREAM_STATES.SUBSCRIBING]: [STREAM_STATES.LIVE, STREAM_STATES.RECONNECTING, STREAM_STATES.FAILED, STREAM_STATES.CLOSED],
