...
const TRANSITIONS = Object.freeze({
  ...
  VALIDATED: Object.freeze({ FILL: ORDER_STATUS.FILLED, REJECT: ORDER_STATUS.REJECTED, CANCEL: ORDER_STATUS.CANCELED }),
...
export function transitionOrder(order, type, { now = () => new Date().toISOString(), reason = "" } = {}) {
  ...
  if (!next) throw new Error(`invalid order transition: ${order.status} -> ${type}`);
