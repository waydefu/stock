...
| R-006 | P1 | Confirmed at baseline; partial schema fix in Phase 3 | AuditLog 只在 memory，reload 消失；class 有 `clear()`；event schema 無 version/id。 | 無法宣稱 durable／append-only audit。 | Baseline：`js/risk.js:70-90`；修復證據：version／eventId／CSV schema test。 | clear 移除、event schema versioned；仍待 persistence scope、durable append-only storage、actor／request id。 |
